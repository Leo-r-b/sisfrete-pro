...
export default function Sidebar({ 
  ...
  // - Para usuários regulares (admin, operador, financeiro): SEMPRE filtra estritamente as abas configuradas na licença.
  // - Para super_admin: se a empresa ativa possui módulos configurados e o super_admin está no ambiente operacional dessa empresa,
  ...
  return (
    <>
      ...
      {/* Sidebar Principal (Desktop recolhível + Mobile gaveta lateral) */}
      <aside className={`
        fixed md:static inset-y-0 left-0 z-50
        ${isCollapsed ? 'md:w-20 w-64' : 'w-64'} flex-shrink-0 bg-slate-900 border-r border-slate-800 flex flex-col h-screen select-none
        transform transition-all duration-300 ease-in-out
        ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
      `}>
        {/* Brand Logo & Collapse Toggle */}
        <div className={`h-16 flex items-center ${isCollapsed ? 'justify-center px-2' : 'justify-between px-4 sm:px-5'} border-b border-slate-800 transition-all`}>
          <div className="flex items-center gap-3 overflow-hidden">
            <div 
              className="h-9 w-9 rounded-xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-emerald-400 flex items-center justify-center shadow-lg shadow-blue-500/20 flex-shrink-0 cursor-pointer"
              onClick={onToggleCollapse}
              title={isCollapsed ? "Expandir menu lateral" : "SisFrete PRO"}
            >
              <Truck className="h-5 w-5 text-white" />
            </div>
            {!isCollapsed && (
              <div className="text-left overflow-hidden">
                <h1 className="font-heading font-bold text-base tracking-tight text-white leading-none truncate">
                  SisFrete <span className="text-emerald-400">PRO</span>
                </h1>
                <p className="text-[9px] text-slate-400 font-medium tracking-wide mt-0.5 truncate">
          ...
          {onToggleCollapse && (
            <button
              onClick={onToggleCollapse}
              className={`p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 hidden md:flex items-center justify-center transition ${isCollapsed ? 'mt-1' : ''}`}
        ...
        {/* Navigation Links */}
        <nav className={`flex-1 ${isCollapsed ? 'px-1.5' : 'px-3'} space-y-3 overflow-y-auto pt-2`}>
                ...
                {saasMasterItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center ${isCollapsed ? 'justify-center px-0 py-2.5' : 'gap-3 px-3.5 py-2.5'} rounded-xl font-bold text-xs sm:text-sm text-left transition-all duration-200 cursor-pointer ${
                        isActive
                    ...
                    >
                      ...
                      {!isCollapsed && <span className="flex-1 text-left leading-snug">{item.label}</span>}
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => handleNavClick(item.id)}
                    title={isCollapsed ? item.label : undefined}
                    className={`w-full flex items-center ${isCollapsed ? 'justify-center px-0 py-2.5' : 'gap-3 px-3.5 py-2.5'} rounded-xl font-medium text-xs sm:text-sm text-left transition-all duration-200 cursor-pointer ${
                      isActive
        ...
        <div className={`p-2.5 sm:p-3 border-t border-slate-800 bg-slate-900/60 ${isCollapsed ? 'flex flex-col items-center gap-2' : ''}`}>
          {!isCollapsed ? (
            ...
            <div className="flex flex-col items-center gap-2">
              <div 
                className="h-8 w-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-xs text-blue-400"
