...
function gerarXmlIbsCbs(creds) {
  ...
  // Tributação
  const imposto = `<imp><ICMS><ICMS00><CST>00</CST><vBC>8500.00</vBC><pICMS>12.00</pICMS><vICMS>1020.00</vICMS></ICMS00></ICMS><IBSCBS><CST>000</CST><cClassTrib>000001</cClassTrib><indDeducao>0</indDeducao><vBC>8500.00</vBC><pIBS>0.10</pIBS><vIBS>8.50</vIBS><pCBS>0.90</pCBS><vCBS>76.50</vCBS></IBSCBS></imp>`;
...
async function testarIbsCbs() {
  ...
  const { xmlCteCompleto, chave44 } = gerarXmlIbsCbs(creds);
  ...
  console.log(`📡 Transmitindo CT-e 4.00 com IBS/CBS para SEFAZ PR...`);
  const res = await postSefaz(sefazUrl, soapEnvelope, creds.keyPem, creds.certPem, creds.caPems);
  console.log(res.data);
}

testarIbsCbs().catch(console.error);
