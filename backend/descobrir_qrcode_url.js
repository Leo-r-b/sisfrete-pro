...
function gerarXmlComQrCode(qrCodeBaseUrl, creds) {
  const cUF = '41';
  const aamm = '2608';
  const mod = '57';
  const serie = 1;
  ...
  const qrCodeFinal = `${qrCodeBaseUrl}?chCTe=${chave44}&amp;tpAmb=2`;
...
async function descobrirQrCodeUrl() {
  ...
  const candidates = [
    ...
    'https://www.fazenda.pr.gov.br/cte/qrcode',
    'http://homologacao.fazenda.pr.gov.br/cte/qrcode',
    'http://homologacao.cte.fazenda.pr.gov.br/cte/qrcode',
    'https://hom.cte.fazenda.gov.br/portal/qrcode.aspx',
    'https://cte.fazenda.pr.gov.br/cte/qrcode',
    'https://dfe.sefa.pr.gov.br/cte/qrcode',
    'https://homologacao.dfe.sefa.pr.gov.br/cte/qrcode'
  ];

  const sefazUrl = 'https://homologacao.cte.fazenda.pr.gov.br/cte4/CTeRecepcaoSincV4';

  for (const baseUrl of candidates) {
    console.log(`\n🧪 Testando QR Code URL: ${baseUrl}`);
    const { xmlCteCompleto, chave44 } = gerarXmlComQrCode(baseUrl, creds);
    ...
    try {
      ...
      const cStatMatch = res.data.match(/<cStat>(\d+)<\/cStat>/);
      const xMotivoMatch = res.data.match(/<xMotivo>([^<]+)<\/xMotivo>/);
      console.log(`📡 Retorno SEFAZ: cStat = ${cStatMatch ? cStatMatch[1] : '?'}, xMotivo = ${xMotivoMatch ? xMotivoMatch[1] : '?'}`);
      if (cStatMatch && cStatMatch[1] === '100') {
        console.log(`🎉🎉🎉 SUCESSO ABSOLUTO! AUTORIZADO PELA SEFAZ! 🎉🎉🎉`);
        console.log(`URL correta: ${baseUrl}`);
        console.log(`🔑 Chave de Acesso: ${chave44}`);
...
descobrirQrCodeUrl().catch(console.error);
